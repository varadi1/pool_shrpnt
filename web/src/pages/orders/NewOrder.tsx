...
export const NewOrder: React.FC = () => {
  ...
  const steps: WizardStepDefinition[] = useMemo(
    () => [
      { key: 'contract', label: 'Szerződés kiválasztása' },
      { key: 'details', label: 'Megrendelés részletei' },
      { key: 'template', label: 'Sablon kiválasztása' },
      { key: 'parts', label: 'Részek konfigurálása' },
      { key: 'partners', label: 'Partner hozzárendelés' },
      { key: 'review', label: 'Áttekintés és küldés' },
    ],
    []
  );
    ...
    switch (stepKey) {
      ...
      case 'details':
        return Boolean(
          ...
          // The order code is generated by the backend. Do not block navigation on it.
          true
        );
      case 'template':
        return true; // Template is optional, can proceed without selecting one
      case 'parts':
        return true; // Parts are optional, can be added later
      case 'partners':
        return true; // Partners are optional, can be added later
      case 'review':
    ...
    if (currentKey === 'review') {
      (async () => {
        ...
        try {
          ...
          const newOrder = {
            id: String(Date.now()),
            code: `EM-2025-PW-${Date.now()}`,
            name: formData.orderName || 'New Order',
            contractName: formData.contractName || 'Test Contract',
            status: 'active' as const,
            createdDate: new Date().toISOString().split('T')[0],
            deadline: formData.endDate ? new Date(formData.endDate).toISOString().split('T')[0] : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            partsCount: formData.parts?.length || 0,
          };
          ...
          // Try API call but don't fail on error
          try {
            ...
            // API call failed, but order saved to localStorage
            console.log('API call failed, order saved locally');
          }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <Title3>Új megrendelés</Title3>
      </div>

      {/* Step content - components implemented in subsequent tasks */}
      <div className={styles.content} role="region" aria-live="polite" data-testid="wizard-content">
        <div style={{ marginBottom: '16px' }}>
          <Title3 aria-label="Aktuális lépés fejléce">
            {currentStep.key === 'contract' && 'Szerződés kiválasztása'}
            {currentStep.key === 'details' && 'Megrendelés részletei'}
            {currentStep.key === 'template' && 'Sablon kiválasztása'}
            {currentStep.key === 'parts' && 'Részek konfigurálása'}
            {currentStep.key === 'partners' && 'Partner hozzárendelés'}
            {currentStep.key === 'review' && 'Áttekintés és küldés'}
          </Title3>
          {infoMessage && (
      ...
      <div className={styles.footer}>
        <WizardNavigation
          ...
          onBack={goBack}
          onNext={goNext}
          submitLabel="Megrendelés küldése"
          onSaveDraft={() => {
            const payload = {
