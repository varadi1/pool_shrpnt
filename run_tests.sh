#!/bin/bash
# Test runner script for poolDRV project

# Set test environment variables
export AZURE_TENANT_ID="test-tenant"
export AZURE_CLIENT_ID="test-client"
export AZURE_CLIENT_SECRET="test-secret"
export POOLDRV_AZURE_TENANT_ID="test-tenant"
export POOLDRV_AZURE_CLIENT_ID="test-client"
export POOLDRV_AZURE_CLIENT_SECRET="test-secret"
export POOLDRV_SHAREPOINT_SITE_URL="https://test.sharepoint.com/sites/test"
export POOLDRV_SHAREPOINT_TENANT_NAME="test"

# Database configuration - using SQLite for tests
export DATABASE_URL="sqlite:///test.db"
export POOLDRV_DATABASE_URL="sqlite:///test.db"
export TEST_DATABASE_URL="sqlite:///test.db"
export ASYNC_TEST_DATABASE_URL="sqlite+aiosqlite:///test.db"

# Run tests
echo "Running poolDRV tests..."
echo "========================="

if [ "$1" == "--story-34" ]; then
    echo "Running Story 3.4 notification tests only..."
    python3 -m pytest api/tests/test_notifications.py api/tests/test_teams_notifications.py api/tests/test_notification_monitoring.py -v
elif [ "$1" == "--all" ]; then
    echo "Running all tests..."
    python3 -m pytest api/tests/ -v
else
    # Run specific test file if provided
    if [ -n "$1" ]; then
        echo "Running tests: $@"
        python3 -m pytest "$@"
    else
        # Default: run story 3.4 tests
        echo "Running Story 3.4 notification tests..."
        python3 -m pytest api/tests/test_notifications.py api/tests/test_teams_notifications.py api/tests/test_notification_monitoring.py -v
    fi
fi