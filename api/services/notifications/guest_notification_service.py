...
class GuestNotificationService:
    ...
    async def send_expiry_warning(
        self, guest: GuestUser, days_until_expiry: int, correlation_id: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            # Format expiry date
            expiry_date = (
                guest.expires_at.strftime("%Y-%m-%d %H:%M") if guest.expires_at else "Unknown"
            )
    ...
    async def send_revocation_notification(
        ...
        try:
            ...
            for locale in ["en", "hu"]:
                ...
                admin_recipients = [
                    {"email": email, "role": "admin"} for email in self.admin_emails
                ]
    ...
    async def send_expiry_notification(
        self, guest: GuestUser, correlation_id: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"guest_id": str(guest.id), "correlation_id": correlation_id},
            )
            return True
