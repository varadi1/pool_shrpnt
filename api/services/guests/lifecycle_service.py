...
class GuestLifecycleService:
    ...
    async def extend_guest_access(
        ...
        if guest.extended_count >= policy.max_extensions:
            raise ValueError(
                f"Guest has reached maximum extensions ({policy.max_extensions}): {guest_id}"
            )

        # Calculate new expiry date
        current_expiry = guest.expires_at or datetime.now(UTC)
        extension_period = extension_days or policy.extension_period_days
    ...
    async def expire_guest(
        ...
        if guest.status in [GuestStatus.EXPIRED.value, GuestStatus.REVOKED.value, GuestStatus.PURGED.value]:
    ...
    async def send_expiry_notifications(
        ...
        for guest in expiring_guests:
            try:
                ...
                days_remaining = (guest.expires_at - datetime.now(UTC)).days if guest.expires_at else 0
