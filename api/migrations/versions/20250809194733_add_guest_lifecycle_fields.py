...
def upgrade():
    ...
    op.add_column(
        "guest_user", sa.Column("extended_count", sa.Integer(), nullable=True, server_default="0")
    )
    op.add_column(
        "guest_user", sa.Column("last_extended_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.create_index(
        op.f("ix_guest_extension_guest_user_id"), "guest_extension", ["guest_user_id"], unique=False
    )

    # Create guest_lifecycle_policy table
    op.create_table(
        "guest_lifecycle_policy",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index(
        op.f("ix_guest_lifecycle_policy_partner_company_id"),
        "guest_lifecycle_policy",
        ["partner_company_id"],
        unique=True,
    )
...
def downgrade():
    # Drop tables
    op.drop_index(
        op.f("ix_guest_lifecycle_policy_partner_company_id"), table_name="guest_lifecycle_policy"
    )
