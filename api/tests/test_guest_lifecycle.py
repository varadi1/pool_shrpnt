...
def sample_partner():
    ...
    return PartnerCompany(id=1, name="Test Partner", short_code="TST", domain="testpartner.com")
...
class TestGuestLifecycleService:
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_policy_existing(
        self, lifecycle_service, mock_session, sample_policy
    ):
        ...
        # Execute
        result = await lifecycle_service.get_or_create_policy(partner_company_id=1)
    ...
    async def test_get_or_create_policy_new(self, lifecycle_service, mock_session):
        ...
        result = await lifecycle_service.get_or_create_policy(
            partner_company_id=1, default_expiry_days=60, max_extensions=2, extension_period_days=30
        )
    ...
    async def test_calculate_expiry_date(self, lifecycle_service, mock_session, sample_policy):
        ...
        expiry_date = await lifecycle_service.calculate_expiry_date(
            partner_company_id=1, from_date=base_date
        )
    ...
    async def test_extend_guest_max_extensions_reached(
        ...
        with pytest.raises(ValueError, match="reached maximum extensions"):
            await lifecycle_service.extend_guest_access(
                guest_id=sample_guest.id, extended_by="admin@company.com", justification="Test"
            )
    ...
    async def test_find_expired_guests(self, lifecycle_service, mock_session):
        ...
        expired_guest = MagicMock(
            expires_at=datetime.now(UTC) - timedelta(days=1), status=GuestStatus.ACCEPTED.value
        )
    ...
    async def test_expire_guest(
        ...
        result = await lifecycle_service.expire_guest(
            guest_id=sample_guest.id, correlation_id="test-correlation"
        )
    ...
    @pytest.mark.asyncio
    async def test_expire_already_expired_guest(
        self, lifecycle_service, mock_session, sample_guest
    ):
    ...
    @pytest.mark.asyncio
    async def test_can_extend_guest(
        self, lifecycle_service, mock_session, sample_guest, sample_policy
    ):
    ...
    async def test_apply_grace_period(self, lifecycle_service, mock_session, sample_guest):
        ...
        # Execute
        result = await lifecycle_service.apply_grace_period(guest_id=sample_guest.id, grace_days=7)
