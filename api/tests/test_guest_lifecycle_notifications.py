...
class TestGuestNotificationService:
    ...
    async def test_send_expiry_warning(
        self, notification_service, mock_guest, mock_partner, mock_session
    ):
        ...
        result = await notification_service.send_expiry_warning(
            guest=mock_guest, days_until_expiry=7, correlation_id="test-123"
        )
    ...
    async def test_send_revocation_notification(
        self, notification_service, mock_guest, mock_partner, mock_session
    ):
    ...
    async def test_send_extension_confirmation(
        self, notification_service, mock_guest, mock_partner, mock_session
    ):
    ...
    async def test_send_expiry_notification(
        self, notification_service, mock_guest, mock_partner, mock_session
    ):
        ...
        result = await notification_service.send_expiry_notification(
            guest=mock_guest, correlation_id="test-999"
        )
    ...
    async def test_error_handling(self, notification_service, mock_guest, mock_session):
        ...
        result = await notification_service.send_expiry_warning(
            guest=mock_guest, days_until_expiry=7
        )
    ...
    async def test_admin_email_configuration(self):
        ...
        with patch(
            "api.services.notifications.guest_notification_service.settings"
        ) as mock_settings:
