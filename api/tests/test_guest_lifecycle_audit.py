...
class TestGuestRevocationAudit:
    ...
    async def test_guest_revoked_audit_event(
        self, guest_service, mock_guest, mock_partner, mock_audit_service, mock_session
    ):
    ...
    async def test_guest_purged_audit_event(
        ...
        await guest_service.purge_revoked_guest(
            guest_id=mock_guest.id, correlation_id=correlation_id
        )
...
class TestGuestExtensionAudit:
    ...
    async def test_guest_extended_audit_event(
        self, lifecycle_service, mock_guest, mock_partner, mock_audit_service, mock_session
    ):
        ...
        with patch.object(
            lifecycle_service, "get_or_create_policy", new_callable=AsyncMock
        ) as mock_get_policy:
...
class TestAuditEventPartnerContext:
    ...
    async def test_all_events_include_partner_context(
        self,
        guest_service,
        lifecycle_service,
        mock_guest,
        ...
        for event_name, method in events_to_test:
            ...
            if event_name == "GUEST_REVOKED":
                ...
                guest_service.graph_auth.get_graph_client.return_value.__aenter__.return_value = (
                    mock_client
                )
...
class TestAuditEventCorrelation:
    ...
    async def test_correlation_id_propagation(self, guest_service, mock_audit_service):
        ...
        with patch.object(guest_service, "revoke_guest", new_callable=AsyncMock) as mock_revoke:
            ...
            bulk_call = [
                c
                for c in mock_audit_service.log_guest_event.call_args_list
                if c[1]["action"] == "GUEST_BULK_REVOKED"
            ][0]
            assert bulk_call[1]["correlation_id"] == correlation_id

    async def test_correlation_id_generation(
        self, guest_service, mock_guest, mock_session, mock_audit_service
    ):
        ...
        await guest_service.revoke_guest(
            guest_id=mock_guest.id, revoked_by="admin@example.com", revocation_reason="Test"
        )
